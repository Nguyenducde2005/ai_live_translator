  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Breadcrumb */}
        <Breadcrumb items={[{ label: t('navigation.workspaces') }]} />
        
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{t('navigation.workspaces')}</h1>
              <p className="text-gray-600">{t('workspaces.manageSlackApps')}</p>
            </div>
            {canCreateSlackApp ? (
              ...
              <div className="text-sm text-gray-500">
                {t('workspaces.maxSlackAppsLimit')}
              </div>
            )}
          </div>
          ...
          {isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="mt-2 text-gray-600">{t('workspaces.loadingSlackApps')}</p>
            </div>
          ) : slackApps.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">{t('workspaces.noSlackApps')}</h3>
              <p className="text-gray-600 mb-4">{t('workspaces.connectFirstSlackApp')}</p>
              {canCreateSlackApp && (
                <Link href={`/${locale}/dashboard/slack-app/create`}>
                  <Button className="bg-black hover:bg-gray-800 text-white flex items-center space-x-2">
                    <Plus className="w-4 h-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {slackApps.map((slackApp) => (
                <div key={slackApp.id} className="bg-gray-50 rounded-lg p-6 border border-gray-200 relative group cursor-pointer hover:shadow-md transition-shadow">
                  ...
                  <Link href={`/${locale}/dashboard/slack-app/${slackApp.id}/edit`} className="block">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">
                          {t('workspaces.teamId')}: {slackApp.team_id || 'N/A'}
                        </p>
                      </div>
                    </div>
                    
                    <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                      <span>{t('workspaces.created')}: {new Date(slackApp.created_at).toLocaleDateString()}</span>
                    </div>
